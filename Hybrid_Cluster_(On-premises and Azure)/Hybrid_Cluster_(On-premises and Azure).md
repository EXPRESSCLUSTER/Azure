...
## Create a cluster
Create the cluster, including the Azure DNS resource, by following the instructions in [section 4.3](https://www.nec.com/en/global/prod/expresscluster/en/doc/guide/HOWTO_AZURE_X53_Windows_EN_01.pdf#page=57) of the Azure configuration guide. Perform the installation steps on the Azure side node and proceed up to the **Custom monitor resource** section. We will not be adding monitor resources. Skip down to **page 67 (of the PDF file), step 5** to complete the configuration. Follow the instructions in following section (4.4) to verify whether the environment is working. See the note about testing by deleting the A record.
