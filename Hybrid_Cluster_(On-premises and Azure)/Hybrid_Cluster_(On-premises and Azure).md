...
Install EXPRESSCLUSTER on the Azure VM using the instructions from [section 4.2.1](https://www.nec.com/en/global/prod/expresscluster/en/doc/manuals/W53_IG_EN_02.pdf#page=62) in the **Installation and Configuration Guide**. If more information on registering a license is needed, click [here](https://www.nec.com/en/global/prod/expresscluster/en/doc/manuals/W53_IG_EN_02.pdf#page=70). Repeat on the on-premises node.

## Create a cluster
Create the cluster, including the Azure DNS resource, by following the instructions in [section 4.3](https://www.nec.com/en/global/prod/expresscluster/en/doc/guide/HOWTO_AZURE_X53_Windows_EN_01.pdf#page=57) of the Azure configuration guide. Perform the installation steps on the Azure side node and proceed up to the **Custom monitor resource** section. We will not be adding monitor resources. Skip down to **page 71 (of the PDF file), step 5 - Applying the settings and starting the cluster** to complete the configuration. Follow the instructions in the following section (4.4) to verify whether the environment is working. See the note about testing by deleting the A record.
...
### On Witness Server
In order to provide NP resolution, a witness server VM needs to be prepared on Azure with a special witness server service. It is best if it is not in the same network or region as the VPN gateway. It also needs a public IP address which can be accessed by each node of the EXPRESCLUSTER cluster, from both the Azure site and on-premises site. In order to set up the witness service, you will need to download Node.js (which is required by the witness server service). A Node.js installation package can be downloaded from the [Nodejs.org page](https://nodejs.org/en/download/). You will also need to locate the witness server service module, **clpwitnessd-\<version\>.tgz**, which can be found in the **_Common\5.3\common\tools\witnessd_** subfolder of the EXPRESSCLUSTER installation package. Copy the Node.js installation package and \.tgz file to the witness server and follow the [installation guide](https://docs.nec.co.jp/software/clustering/expresscluster_x/x53/ecx_x53_windows_en/W53_RG_EN/W_RG_08.html#witness-server-service).
