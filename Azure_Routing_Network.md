...
## Advantage
There is no need to rely on DNS with Azure. Since there is no need to rely on DNS, it can be used in environments where quick responses are required.
If you use this routing, you can use the secondary IP to dynamically fail over this IP to this region.
...
If there are multiple next hop entries with the same address prefix, Azure selects the routes in the following order:
...
> **Note:** The source document ends here with no diagram or further
> implementation detail included. This section needs the referenced
> picture (and likely accompanying implementation steps) added before
> it's complete.
